...
import cn.edu.fan.himalaya.data.ISubDaoCallback;
import cn.edu.fan.himalaya.data.SubscriptionDao;
...
public class SubscriptionPresenter implements ISubscriptionPresenter, ISubDaoCallback {
    ...
    private void listSubscriptions(){
        Observable.create(new ObservableOnSubscribe<Object>() {
            public void subscribe(@NonNull ObservableEmitter<Object> var1) throws Exception {
                //只调用不处理
    ...
    public void onAddResult(final boolean isSuccess) {
        Log.d(TAG,"listSubscriptions after add...");
        listSubscriptions();
        //添加结果的回调
        BaseApplication.getsHandler().post(new Runnable() {
            @Override
            public void run() {
                Log.d(TAG,"update ui for add result.");
                for (ISubscriptionCallback callback : mCallbacks) {
